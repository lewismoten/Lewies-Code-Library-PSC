...
' -----------------------------------------------------------------------------
		Private Sub Class_Initialize()
			...
			mStrConnectionString	= "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" & Server.MapPath("gallery.mdb")
			mStrUserID				= ""
			mStrPassword			= ""
			mBlnIsSQLServer			= ""
...
' -----------------------------------------------------------------------------
		Public Function ExecuteSQL(ByRef asSQL)
			...
			If Err Then
				...
				SQLDebugger "ExecuteSQL()", Err.Description
				
				' Append error to internal error collection
				mStrErrors = mStrErrors & ";" & Err.Description
...
' -----------------------------------------------------------------------------
		Public Function OpenDatabase()
			...
			If mStrConnectionString = "" Then
				...
				mStrErrors = mStrErrors & ";No connection string has been defined."
...
' -----------------------------------------------------------------------------
		Public Function SetData(ByRef asSQL, ByRef avDataAry)
			...
			If asSQL = "" Then
				SQLDebugger "SetData()", "No SQL Provided!!!"
				Exit Function
			End If

			If Err Then
				SQLDebugger "Pre-Existing Errors", Err.Description
				Exit Function
			End If
			' Prepare for errors when executing query
			'On Error Resume Next
			...
			If Err Then
				...
				SQLDebugger "SetData()", Err.Description
			...
			ElseIf Not mObjRs.State = ADODB.adStateOpen Then
				...
				' Append error to internal error collection
				mStrErrors = mStrErrors & ";Recordset could not be opened."
